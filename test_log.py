...
def astrology_data(api_url):
    try:
        logger.info("Calling astrology_data with URL: %s", api_url)
...
def format_date(date_str):
    logger.info("Formatting date: %s", date_str)
...
def get_current_and_next_date():
    ...
    logger.info("Current date: %s, Next date: %s", current_date, next_date)
...
def multiple_api_callings(user_prompt, personId, lang, partnerId):
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                "name": "north_match",
                "description": "Perform matchmaking analysis using the North Indian astrology method.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "api_url": {
                            "type": "string",
                            "description": "URL or unique identifier for accessing the matchmaking analysis."
                        }
                    },
                    "required": ["api_url"]
                }
        ...
        },
        {
            ...
            "function": {
                "name": "dashakoot",
                "description": "Perform matchmaking analysis using the Dashakoot method.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "api_url": {
                            "type": "string",
                            "description": "URL or unique identifier for accessing the Dashakoot matchmaking analysis."
                        }
                    },
                    "required": ["api_url"]
                }
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "Aggregate the results of multiple matchmaking methods to provide an overall compatibility score.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "api_url": {
                            "type": "string",
                            "description": "URL or unique identifier for accessing the aggregate matchmaking analysis."
                        }
                    },
                    "required": ["api_url"]
                }
    ...
    logging.debug("Response message: %s", response_message)

    tool_calls = getattr(response_message, 'tool_calls', [])
    logging.debug("Tool calls extracted: %s", tool_calls)
    ...
    if tool_calls:
        ...
        for tool_call in tool_calls:
            ...
            logging.info("Processing tool call: %s with arguments %s", function_name, function_args)
        ...
        try:
            ...
            return {"error": "Failed to generate final response from OpenAI"}

        logging.debug("Final response from LLM: %s", second_response.choices[0].message.content)
...
def get_astrology_prediction():
    ...
    logging.info("Received request with user details: %s and partner details: %s", user_details, partner_details)
